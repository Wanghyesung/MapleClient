...
namespace W
{
	...
	void Player::child_lateupdate()
	{
		...
		for (GameObject* pObj : m_vecChildObj)
		{
			//pObj->GetComponent<Transform>()->SetDirectPosition(GetComponent<Transform>()->GetPosition());
			pObj->LateUpdate();
		}
	}
	...
	void Player::init_attack_effect()
	{
		for (int i = 0; i < 5; ++i)
		{
			...
			Light* pLight = pEffect->AddComponent<Light>();
			pLight->SetType(eLightType::FadeOut);
			pLight->SetColor(Vector4(0.8f, 0.8f, 0.8f, 0.0f));
	...
	void Player::send_equip(Equip* _pEquip, UINT _iEquipType)
	{
		...
		UCHAR cEquipID = (UCHAR)_iEquipType;
		UINT iEquipInfo = {};
		iEquipInfo = (cScene << 24) | (cLayer << 16) | (cPlayerID << 8 | cEquipID);
		
		//아이템 부위 , 장비 부위, 입는건지 해제하는건지, 아이템 아이디
		if (_pEquip != nullptr)
		{
			UINT iItemID = _pEquip->GetItemID();
			pkt.set_item_id(iItemID);
		}
		else
		{
			pkt.set_item_id(0);
		...
		pkt.set_scene_layer_playerid_equipid(iEquipInfo);
